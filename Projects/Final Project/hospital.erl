% hospital.erl - Distributed Adventure Game Location Hospital
...
start(ServerNode) ->
   ...
   LocPid = spawn(hospital, locationLoop, []),
   ...
   register(hospital, LocPid),
   io:fwrite(", registered as ~w.~n",[hospital]),
   % Send ourselves to the gameServer.
   io:fwrite("~sNotifying server on node ~w.~n",[?id, ServerNode]),
   {gameServer, ServerNode} ! {node(), registerNewLocation, hospital},
   % Initialize server monitoring.
   hospital ! {monitor, ServerNode},
   ok.
...
locationLoop() ->
   receive
      ...
      {_FromNode, enter, GameClientNode}  ->
         io:fwrite("~sA gameClient on ~w is entering hospital.~n",[?id, GameClientNode]),
         {gameClient, GameClientNode} ! {node(), describe()},
         locationLoop();
		 
      {_FromNode, pickup, GameClientNode, Inventory}  ->
         io:fwrite("~sA gameClient on ~w is picking up an M4.~n",[?id, GameClientNode]),
...
%--------
describe() ->
   io_lib:format("(4) You find yourself in a hospital where soldiers are being treated.", []).
   
pickupItem(Inventory) ->
	Inventory ++ m4.

pickupDescription() ->
	io_lib:format("You have picked up an M4 assault rifle.", []).