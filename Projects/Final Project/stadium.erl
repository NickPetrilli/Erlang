% stadium.erl - Distributed Adventure Game Location Stadium
...
start(ServerNode) ->
   ...
   LocPid = spawn(stadium, locationLoop, []),
   ...
   register(stadium, LocPid),
   io:fwrite(", registered as ~w.~n",[stadium]),
   % Send ourselves to the gameServer.
   io:fwrite("~sNotifying server on node ~w.~n",[?id, ServerNode]),
   {gameServer, ServerNode} ! {node(), registerNewLocation, stadium},
   % Initialize server monitoring.
   stadium ! {monitor, ServerNode},
   ok.
...
locationLoop() ->
   receive
      ...
      {_FromNode, enter, GameClientNode}  ->
         io:fwrite("~sA gameClient on ~w is entering stadium.~n",[?id, GameClientNode]),
         {gameClient, GameClientNode} ! {node(), describe()},
         locationLoop();
		 
      {_FromNode, pickup, GameClientNode, Inventory}  ->
         io:fwrite("~sA gameClient on ~w is picking up a UAV.~n",[?id, GameClientNode]),
...
%--------
describe() ->
   io_lib:format("(0) You find yourself inside a large football stadium in the center of the map.~nPick up the uav in front of you.", []).
   
pickupItem(Inventory) ->
	Inventory ++ uav.
	
pickupDescription() ->
	io_lib:format("You have picked up a uav. Use it for aerial coverage of the enemies locations.", []).