...
start(ServerNode) ->
   ...
   LocPid = spawn(superstore, locationLoop, []),
   ...
   register(superstore, LocPid),
   io:fwrite(", registered as ~w.~n",[superstore]),
   % Send ourselves to the gameServer.
   io:fwrite("~sNotifying server on node ~w.~n",[?id, ServerNode]),
   {gameServer, ServerNode} ! {node(), registerNewLocation, superstore},
   % Initialize server monitoring.
   superstore ! {monitor, ServerNode},
   ok.
...
locationLoop() ->
   receive
      ...
      {_FromNode, enter, GameClientNode}  ->
         io:fwrite("~sA gameClient on ~w is entering superstore.~n",[?id, GameClientNode]),
         {gameClient, GameClientNode} ! {node(), describe()},
         locationLoop();
		 
      {_FromNode, pickup, GameClientNode, Inventory}  ->
         io:fwrite("~sA gameClient on ~w is picking up armor plates.~n",[?id, GameClientNode]),
...
%--------
describe() ->
   io_lib:format("(1) You have come to a large supermarket...", []).
  
pickupItem(Inventory) ->
	Inventory ++ armor.
   
pickupDescription() ->
	io_lib:format("You have picked up armor plates", []).