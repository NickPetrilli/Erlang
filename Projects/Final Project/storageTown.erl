...
start(ServerNode) ->
   ...
   LocPid = spawn(storageTown, locationLoop, []),
   ...
   register(storageTown, LocPid),
   io:fwrite(", registered as ~w.~n",[storageTown]),
   % Send ourselves to the gameServer.
   io:fwrite("~sNotifying server on node ~w.~n",[?id, ServerNode]),
   {gameServer, ServerNode} ! {node(), registerNewLocation, storageTown},
   % Initialize server monitoring.
   storageTown ! {monitor, ServerNode},
   ok.
...
locationLoop() ->
   receive
      ...
      {_FromNode, enter, GameClientNode}  ->
         io:fwrite("~sA gameClient on ~w is entering storageTown.~n",[?id, GameClientNode]),
         {gameClient, GameClientNode} ! {node(), describe()},
         locationLoop();

      {_FromNode, pickup, GameClientNode, Inventory}  ->
         io:fwrite("~sA gameClient on ~w is picking up an RPG.~n",[?id, GameClientNode]),
...
%--------
describe() ->
   io_lib:format("(10) You find yourself in an outdoor storage facility with garages full of miscellaneous items.~nYou see that an enemy sniper has you scoped. Shoot him!", []).
   
pickupItem(Inventory) ->
	Inventory ++ rpg.
	
pickupDescription() ->
	io_lib:format("You have picked up an rpg. Load and shoot missles at the enemies!", []).